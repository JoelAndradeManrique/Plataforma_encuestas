...
<body>
  ...
  <script>
    ...
    $(function () {
        ...
        const modoRespuesta = "<?php echo $modo_respuesta; ?>";
        ...
        $form.on('submit', function(e) {
            ...
            $form.find('.pregunta-item').each(function() {
                ...
                // Buscar inputs requeridos DENTRO de esta pregunta
                ...
                if ($requiredInputs.length > 0) {
                    if ($requiredInputs.is(':radio')) {
                        ...
                        if ($(`input[name="${name}"]:checked`).length === 0) {
                            formValido = false;
                            $preguntaItem.css('border-color', 'red');
                        }
                    } else if ($requiredInputs.is('textarea')) {
                        if (!$requiredInputs.val().trim()) {
                            formValido = false;
                            $requiredInputs.css('border-color', 'red');
                            $preguntaItem.css('border-color', 'red'); // Resaltar todo el bloque
                        } else {
                            $requiredInputs.css('border-color', '#ccc');
                        }
                    }
                    // Añadir más validaciones si es necesario (ej. checkboxes)
                }
            });


            if (!formValido) {
                Swal.fire("Atención", "Por favor responde todas las preguntas marcadas antes de enviar.", "warning");
                return;
            }
            ...
            Swal.fire({
                ...
                // Procesar datos para la API (Misma lógica que antes)
